...
func (h *HttpReqFunc) Get(url string, params string) string {
	...
	if err != nil {
		log.Warning("[JSVM] Error while sending GET request: ", err)
		return ""
	}
	return string(resp)
}
...
func (h *HttpReqFunc) GetJson(url string, params string) map[string]any {
	...
	if err != nil {
		log.Warning("[JSVM] Error while parsing JSON: ", err)
		return nil
	}
	return data
}
...
func (h *HttpReqFunc) Post(url string, params any) string {
	...
	if err != nil {
		log.Warning("[JSVM] Error while sending POST request: ", err)
		return ""
	}
	return string(resp)
}
